...
let rec string_of_exp = function
  ...
  | For(id, e1, e2, e3) ->
    Printf.sprintf "for %s := %s to %s do %s done" (Symbol.name id) (string_of_exp e1) (string_of_exp e2) (string_of_exp e3)
  | LetExp(ds, es) ->
    Printf.sprintf "let\n%s\nin\n%s\nend\n"
      (String.concat "\n" (List.map string_of_decl ds))
      (String.concat "\n" (List.map string_of_exp es))
  | Array(typ, size, init) -> Printf.sprintf "%s[%s] of %s" (Symbol.name typ) (string_of_exp size) (string_of_exp init)
  | Record(typ, fields) ->
    Printf.sprintf "%s { %s }"
      (Symbol.name typ)
      (String.concat "; "
...
and string_of_lvalue = function
  | Ident v -> Symbol.name v
  | ArrayAccess(l, e) -> Printf.sprintf "%s[%s]" (string_of_lvalue l) (string_of_exp e)
  | RecordAccess(l, id) -> string_of_lvalue l ^ "." ^ Symbol.name id
...
and string_of_type_spec = function
  | TypeId i -> Symbol.name i
  | TypeArray i -> "array of " ^ Symbol.name i
  | TypeRecord tyfields -> "{" ^ string_of_tyfields "; " tyfields ^ "}"

and string_of_decl = function
  | VarDecl (i, None, e) -> Printf.sprintf "var %s := %s" (Symbol.name i) (string_of_exp e)
  | VarDecl (i, Some t, e) -> Printf.sprintf "var %s: %s := %s" (Symbol.name i) (Symbol.name t) (string_of_exp e)
  | TypeDecl ty_decls ->
    String.concat "\n"
      (List.map (fun (i, ts) ->
        Printf.sprintf "type %s = %s" (Symbol.name i) (string_of_type_spec ts))
         ty_decls)
  | FunDecl fun_decls ->
    String.concat "\n"
